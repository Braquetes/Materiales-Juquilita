...
if(isset($user) AND isset($pass)){
    ...
    if($log->num_rows > 0){//Encontro un registro igual
        while($row = mysqli_fetch_array($log)){ //Este arreglo trae todos los campos de la tabla para utilizarlos despues
            ...
            $antiguo_codigo = mysqli_query($conexion, "DELETE FROM `empleado_codigo` WHERE empleado =".$row['id']);
            $codigo_empleado = mysqli_query($conexion, "INSERT INTO `empleado_codigo` (`codigo`, `empleado`) VALUES ('".$random."', '".$row["id"]."');");
        }
