...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $consulta = "SELECT * FROM carrito inner join productos on productos.id = carrito.Id_producto LEFT JOIN empleados on empleados.id = carrito.Vendedor where Id_carro > 59 LIMIT 1";
        ...
        while ($row = $respuesta->fetch_assoc()) {
            ...
            $this->Cell(12);
            // Título
            $this->Cell(40, 0, utf8_decode("Vendedor: ".$row['nombres']." " . $row['apellido_p'] ." " . $row['apellido_m'] . " "), 0, 0, 'C', 0);
            // Salto de línea
            $this->Ln(5);
            //Movernos a la izquierda
        }
    ...
    function Footer()
    {
        ...
        $mostrarClientes = mysqli_query($conexion, "SELECT * FROM codigo");
        if ($mostrarClientes->num_rows > 0) {
            while ($row  = mysqli_fetch_array($mostrarClientes)){ 
                $rows = $row["codigo"];
                $this->Cell(10, 5, utf8_decode("" .$rows. ""), 0, 0, 'C', 0);
                $this->Ln(5);
                $this->Cell(26);
            }
        }
...
$pdf = new PDF();
//Tamaño de papel
$pdf->AddPage('portrait', array(150,200));
//Tamaño de letra
$pdf->SetFont('Arial', '', 8);
...
while($row = $respuesta-> fetch_assoc()){
    ...
    */
    ...
    require("../BaseDatos/Conexion.php");
    $xdxd = mysqli_query($conexion, "INSERT INTO `codigo` (`Id_producto`, `codigo`,`id_ven`) VALUES ('" . $producto . "', '" . $random . "','".$ID."');");
